...
impl FromStr for Equation {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (result_str, rest) =
            s.split_once(':').ok_or_else(|| eyre!("{s}: missing delimiter '|'"))?;
        let result = result_str.parse::<i64>()?;
        let terms =
            rest.split_whitespace().map(|t| t.parse::<i64>()).collect::<Result<Vec<_>, _>>()?;
        if terms.len() < 2 {
...
fn main() -> Result<()> {
    ...
    let equations = body
        ...
        .enumerate()
        .map(|(lineno, l)| {
            l.parse::<Equation>().wrap_err_with(|| format!("{fname}:{}", lineno + 1))
        })
        .collect::<Result<Vec<_>, _>>()?;
    let first_cal: i64 = equations
        .iter()
        .filter_map(|eq| {
            if eq.is_solvable(&[Operator::Plus, Operator::Multiply]) {
                Some(eq.result)
            } else {
                None
            }
        ...
        .sum();
    println!("{first_cal}");
    let second_cal: i64 = equations
        .iter()
        .filter_map(|eq| {
            if eq.is_solvable(&[Operator::Plus, Operator::Multiply, Operator::Concatenate]) {
                Some(eq.result)
            } else {
                None
            }
