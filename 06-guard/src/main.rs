...
fn main() -> Result<()> {
    ...
    while in_bounds(guard_pos, (rows, columns)) {
        ...
        visited.slice_mut(slice).slice_mut(to_obs).map_inplace(|es| {
            es.insert(dir);
        });
        ...
        .filter(|(block_pos, es)| {
            ...
            while in_bounds(guard_pos, (rows, columns)) {
                ...
                if !hit_obs {
                    ...
                    let is_loop =
                        visited.slice(slice).slice(to_obs).iter().any(|es| es.contains(dir));
                    if is_loop {
